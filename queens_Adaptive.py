...
class GATest():
    ...
    def OrderWeights(self,St):
        ...
        for i in range(self.PopulationSize):
            Weights[i] = self.Fittnes( self.Population[i,:] )
    ...
    def AdaptiveSolver(self):
        self.ReInitPopulation() 
        for St in range(self.NumIteration):
            #calc weights
            Indices, Weights =self.OrderWeights(St)
...
BaseLinePlotingData = np.zeros((len(BasicParameteresCombination),repeat,NumGeneration))

for PInd,BasicParameteres in enumerate(BasicParameteresCombination):
    NumCities = BasicParameteres[0]
    PopulationSize = BasicParameteres[1]
    GA=GATest(NumCities,PopulationSize,NumGeneration)
    for r in range(0,repeat):
        RandomMinWeights=GA.AdaptiveSolver()
        Baseline[r,:]=RandomMinWeights

    BaseLinePlotingData[PInd] = Baseline 